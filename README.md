...
| 3 | `Mcpd::Tools` | `lib/Tools.stk` | 20 | `fs_read` · `fs_list` · `fs_grep` · `fs_find` (recursive, capped) · `fs_stat` · `fs_head` (first N lines) · `fs_tail` (last N lines) · `fs_exists` · `fs_lines` (wc) · `fs_write` · `fs_append` · `fs_mkdir` · `fs_copy` · `sh_exec` (allowlist) · `env_get` · `time_now` · `sys_info` · `all` (`readonly` mode) · `jail` (root confinement) · `names` |
