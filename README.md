...
| 3 | `Mcpd::Tools` | `lib/Tools.stk` | 28 | `fs_read` · `fs_list` · `fs_grep` · `fs_find` (recursive, capped) · `fs_stat` · `fs_readlink` (symlink target) · `fs_du` (recursive size) · `fs_head` (first N lines) · `fs_tail` (last N lines) · `fs_slice` (line range) · `fs_hash` (checksum) · `fs_exists` · `fs_lines` (wc) · `fs_write` · `fs_append` · `fs_mkdir` · `fs_touch` (empty file / mtime) · `fs_rmdir` · `fs_delete` · `fs_copy` · `fs_move` · `sh_exec` (allowlist) · `env_get` · `time_now` · `sys_info` · `all` (`readonly` mode) · `jail` (root confinement) · `names` |
