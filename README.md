...
|---|--------|------|----:|------------|
| 1 | `Mcpd::Schema` | `lib/Schema.stk` | 11 | `tool` (validated spec) · `check_args` (type-checked args) · `validate_all` (unique names, full specs) · `types` · `param_names` · `to_json_schema` (MCP inputSchema) · `from_json_schema` (inputSchema → params, inverse) · `to_tool_list` (MCP tools/list payload) · `tool_summary` · `describe` (catalog) · `rename` |
...
| 4 | `Mcpd::Client` | `lib/Client.stk` | 9 | `text` (envelope → string) · `texts` (unjoined blocks) · `content_types` · `content_of_type` (full blocks of a type) · `call_text` (call + extract) · `is_error` · `error_message` (strip prefix) · `tool_names` · `has_tool` |
